...
console.log(state[0].body)
   ...
   <div className="edit-profile-banner-avatar"><div className={"edit-avatar-value"}><InlineEdit className="postTitle" text= {state[0].author.name}  onSetText= "OnSetText" item={"Banner, click to edit"}/> </div>
...
//value input state problem, there are values that are too far, we have reach problem
        ...
        <div className="p-4 edit-post-container image-edit-container border-bottom border-4">  <InlineEdit className="postMedia" id ={ "image" + item.id} text= {item.media }  onSetText= {OnSetText} item=  {item.media ?   <img className="edit-image-value" id={item.media} src={item.media}/> : "Add Image Here" }   /></div> 

      
        <div className="container d-flexbox align-items-center justify-content-center mb-5 " id={ "post" + item.id}><div>{item.title}</div>

       <h2><InlineEdit className="p-4 edit-post-container postTitle border-bottom border-4" text= {item.title} id={"title" + item.id}  onSetText= {OnSetText} item={item.title}/> </h2>
     
       <InlineEdit className="p-4 edit-post-container postBody border-bottom border-4" text= {item.body}  onSetText= {OnSetText} item={item.body}/> 
       <InlineEdit className="p-4 edit-post-container postTags border-bottom border-4" text= {item.tags}  onSetText= {OnSetText} item={"no tags"}/>
