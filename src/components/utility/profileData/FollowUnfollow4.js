...
//we havde done it, values are correct and if else now goes straighht to profile values array needed,
...
//console.log(followedProfiles2)
   ...
   if( item.name == props.currentProfile) {
      
     // console.log(item.name == props.currentProfile)
const arrayValue = [];

const newArray = arrayValue.push(item.name == props.currentProfile)
...
const FollowProfile = () => {
   const awaitData = async() => {
      ...
      //all we need is to change state on request so it is updated for next async await request
         ...
         RecyclableFetch2("/social/profiles/"+ props.currentProfile + "/" + ternaryFollow , "PUT", undefined, defaultHeaders)
...
return <div className="follow-container-pill p-0"  ><button style={{marginTop: "-25px"}} className="follow-button" onClick={FollowProfile}>{booleanState}</button></div>
